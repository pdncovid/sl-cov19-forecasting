...
def clip_dist(data, data_list, clip_percentages):
    data = np.array(data)
    idx = np.logical_or(data < np.quantile(data, clip_percentages[0] / 100),
                        data >= np.quantile(data, (100 - clip_percentages[1]) / 100))
    data = np.delete(data, idx)
    list1 = [data]
    for list0 in data_list:
        list1.append(np.delete(list0, idx, axis=0))
    return list1
...
def undersample3(x_data, y_data, x_feats, count_h, count_l, num_h, num_l, power_l, power_h, power_penalty,
    ...
    if clip:
        [samples_x_mean, samples_x, samples_y, samples_f] = clip_dist(samples_x_mean, [samples_x, samples_y, samples_f],
                                                                      clip_percentages)
...
def undersample_random(x_data, y_data, x_feats, ratio, clip, clip_percentages, country, PLOT, savepath=None):
