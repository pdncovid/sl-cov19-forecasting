...
def fix_nan_negative(data, daily=False):
    data_ = np.nan_to_num(data)
    if daily:
        data_daily = np.copy(data_)
        data_daily[data_daily < 0] = 0
        data_total = np.cumsum(data_daily)
    else:
        data_total = np.copy(data_)
        for i in range(data_total.shape[0]):
            for k in range(data_total.shape[1] - 1):
                if data_total[i, k + 1] < data_total[i, k]:
                    data_total[i, k + 1] = data_total[i, k]
        data_daily = np.diff(data_total, axis=1)
    return data_total, data_daily
        ...
        if DATASET == "SL":
            ...
            features = pd.concat(
                [population, lat, lon, pop_density, labour_total, labour_skilled, labour_unskilled, labour_agri,
                 unemployment, poverty, spending_total, spending_food, spending_other, spending_ratio, internet,
                 education],
            ...
            features = pd.concat([population], axis=1, join="inner").rename(
                columns={'cqr_census_2010_count': 'Population'})

            # fixing the confirmed cases dataset (negative gradients)
            confirmed_cases, daily_cases = fix_nan_negative(confirmed_cases)
            ...
            confirmed_arr, daily_arr = fix_nan_negative(confirmed_arr)
            ...
            dataset_path = os.path.join(path, "KR")
            df = pd.read_csv(os.path.join(dataset_path, "covid_korea_n.csv"))
            columns_ = df.columns.tolist()
            idx_start = columns_.index('SEOUL')
            START_DATE = df.iloc[0, 0]
            df_confirmed = df.iloc[:, idx_start:-1].transpose()
            region_names = df_confirmed.index.tolist()
            confirmed_cases = df_confirmed.values.astype(int)
            confirmed_cases, daily_cases = fix_nan_negative(confirmed_cases)

        elif DATASET == 'Indonesia' or DATASET == 'ID':
            dataset_path = os.path.join(path, "Indonesia")
            df = pd.read_csv(os.path.join(dataset_path, "kawalcovid-data.csv"), index_col=0)
            START_DATE = df.iloc[0, 0]
            df_confirmed = df.iloc[:, :-1].transpose()
            df_confirmed = df_confirmed.dropna(axis=1)
            region_names = df_confirmed.index.tolist()
            confirmed_cases = df_confirmed.values.astype(int)
            ...
            confirmed_cases, daily_cases = fix_nan_negative(daily_cases, daily=True)

        elif DATASET == 'KZ' or DATASET == 'Kazakhstan':
            dataset_path = os.path.join(path, "KZ")
            _df = pd.read_csv(os.path.join(dataset_path, "cases_kazakhstan.csv"))
            region_idx = _df.index.tolist()
            column_name = 'name'
            date_name = 'date'
            covid_name = 'cumulative_cases'
            ...
            df_time = df_time[df_time.index.notnull()]
            # remove unspecified rows
            remove_rows = ['Outside mainland Norway', '', ' ', 'nan', 'Nan', 'NOT SPECIFIED', 'Non spécifié',
                           'Repatriierte']
            for word in remove_rows:
                if word in df_time.index:
                    df_time = df_time.drop(index=word)
...
def load_data_eu(country='DEU', provinces=True,
    ...
    if country == 'Italy' or country == 'IT':
    ...
    # fill nan values
    df_time = df_time.fillna(value=0)
    confirmed_cases_eu = df_time.values.astype(np.float64)
    confirmed_cases_eu, daily_cases_eu = fix_nan_negative(confirmed_cases_eu)
    start_date = df_time.columns[0]
    features = pd.DataFrame(columns=['Population'],
                            index=df_time.index)  # todo population ignored, features ignored now
    features['Population'] = 1e6

    return {
        "region_names": df_time.index,
        "confirmed_cases": confirmed_cases_eu,
        "daily_cases": daily_cases_eu,
        "features": features,
        "START_DATE": start_date,
