...
def load_data(DATASET, path="/content/drive/Shareddrives/covid.eng.pdn.ac.lk/COVID-AI (PG)/spatio_temporal/Datasets"):
  if DATASET == "Sri Lanka":
    ...
    features = pd.concat([population, lat, lon, pop_density, labour_total, labour_skilled, labour_unskilled, labour_agri, unemployment, poverty, spending_total, spending_food, spending_other, spending_ratio, internet, education], axis=1,join="inner").rename(
  ...
  if DATASET == "Texas":
    ...
    # dataframes
    df_confirmed = pd.read_csv(os.path.join(dataset_path,"Texas COVID-19 Case Count Data by County.csv"),skiprows=2, nrows=254) # https://dshs.texas.gov/coronavirus/AdditionalData.aspx
    df_population = pd.read_csv(os.path.join(dataset_path,"2019_txpopest_county.csv"),header=0) # https://demographics.texas.gov/Resources/TPEPP/Estimates/2019/2019_txpopest_county.csv
    ...
    print('confirmed cases shape:',confirmed_cases.shape,'  daily cases shape:', daily_cases.shape,'  population shape:', population.shape)
